-- Migration: Remove auth.users dependencies for anonymous access (SAFE VERSION)
...
-- Update parties table - convert UUID to TEXT
-- If there's existing UUID data, convert it to text first
DO $$ 
BEGIN
  ...
  IF EXISTS (
    ...
    ALTER TABLE parties ALTER COLUMN created_by TYPE TEXT USING created_by::TEXT;
  ELSE
    -- Already TEXT, just make sure
...
BEGIN
  IF EXISTS (
    ...
    ALTER TABLE party_members ALTER COLUMN user_id TYPE TEXT USING user_id::TEXT;
  ELSE
...
BEGIN
  IF EXISTS (
    ...
    ALTER TABLE user_swipes ALTER COLUMN user_id TYPE TEXT USING user_id::TEXT;
  ELSE
